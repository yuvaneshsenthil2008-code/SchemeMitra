...
def set_artifact_available(client_id: str, artifact_type: str, notes: Optional[str] = None) -> Dict[str, Any]:
    if not client_id or not str(client_id).strip():
        raise ValueError("client_id is required for artifact persistence.")
    
    atype = str(artifact_type).strip().upper()
    now_iso = datetime.datetime.now(datetime.timezone.utc).isoformat()
    disp_name = SHARED_ARTIFACT_NAMES.get(atype, atype.replace("_", " ").title())
        ...
        with conn:
            conn.execute("""
                INSERT INTO user_artifacts (
                    client_id, artifact_type, status, display_name, notes, created_at, updated_at
                ) VALUES (?, ?, 'AVAILABLE', ?, ?, ?, ?)
                ON CONFLICT(client_id, artifact_type) DO UPDATE SET
                    status = 'AVAILABLE',
                    display_name = excluded.display_name,
                    notes = excluded.notes,
                    updated_at = excluded.updated_at
            """, (client_id, atype, disp_name, notes or "", now_iso, now_iso))
    finally:
        conn.close()
...
def remove_artifact(client_id: str, artifact_type: str) -> Dict[str, Any]:
    if not client_id or not str(client_id).strip():
        raise ValueError("client_id is required for artifact persistence.")
    ...
    try:
        with conn:
            conn.execute("""
                ...
                WHERE client_id = ? AND artifact_type = ?
            """, (client_id, atype))
    finally:
        conn.close()
...
def get_user_artifacts(client_id: str) -> List[Dict[str, Any]]:
    if not client_id or not str(client_id).strip():
        return []
    ...
    try:
        cursor = conn.execute("""
            SELECT artifact_type, status, display_name, notes, updated_at
            FROM user_artifacts
